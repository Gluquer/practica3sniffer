...
from struct import *
def getFlagsTCP(packet):
    FlagTCP_URG = {0:"Urgent Flag         : 0",1: "Urgent Flag         : 1"}
    FlagTCP_ACK = {0:"Acknowledgement Flag: 0",1: "Acknowledgement Flag: 1"}
    FlagTCP_PSH = {0:"Push Flag           : 0",1: "Push Flag           : 1"}
    FlagTCP_RST = {0:"Reset Flag          : 0",1: "Reset Flag          : 1"}
    FlagTCP_SYN = {0:"Synchronise Flag    : 0",1: "Synchronise Flag    : 1"}
    FlagTCP_FIN = {0:"Finish Flag         : 0",1: "Finish Flag         : 1"}
    ...
    new_line = '\n\t|-'
    Flags = new_line + FlagTCP_URG[URG] + new_line + FlagTCP_ACK[ACK] + new_line + FlagTCP_PSH[PSH] + new_line + FlagTCP_RST[RST] + new_line + FlagTCP_SYN[SYN] + new_line + FlagTCP_FIN[FIN]
    return Flags
    ...
    def capture_packets(self):
        while True:
            ...
            if self.ip is not True:
                ...
                # create info
                addrinfo = [
                    '\n\t|-Destination Address : {}'.format(self.mac_addr(packet[0:6])),
                    '\t|-Source  Address       : {}'.format(self.mac_addr(packet[6:12])),
            ...
            headerinfo = [
                ...
                '\t|-IP Header Length (IHL) : {}'.format(ihl),
                '\t|-Type of Service (TOS)  : '+str(ip_tos),
                '\t|-IP Total Length        : {}'.format(ip_len)+' Bytes(Size of Packet)',
                '\t|-Identification         : {}'.format(ip_id),
                '\t|-TTL                    : {}'.format(ip_ttl),
                '\t|-Protocol               : {}'.format(protocol),
                '\t|-Checksum               : {}'.format(ip_sum),
                '\t|-Source Address IP      : {}'.format(s_addr),
                '\t|-Destination Address IP : {}'.format(d_addr)]
            ...
            if protocol == 6:
                ...
                tcpinfo = [
                    ...
                    '\t|-Sequence Number    : {}'.format(sequence),
                    '\t|-Acknowledge Number : {}'.format(acknowledgement),
                    '\t|-Header Length      : {}'.format(tcph_length)+' DWORDS or '+str(tcph_length*32//8)+' BYTES',
                    "\t|-Urgent Flag        : {}".format(tcph_flags)+getFlagsTCP(tcph_flags),
                    "\t|-Window             : {}".format(tcph_window_size),
                    "\t|-Checksum           : {}".format(tcph_checksum),
                    "\t|-Urgent Pointer     : {}".format(tcph_urgent_pointer)
                ]
                print('\nTCP Header' + '\n'.join(tcpinfo))
